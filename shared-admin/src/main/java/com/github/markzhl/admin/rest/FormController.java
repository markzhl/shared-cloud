...
import com.github.markzhl.admin.biz.FormBiz;
...
@Controller
@RequestMapping("form")
public class FormController extends BaseController<FormBiz, Form> {
    ...
    public TableResultResponse<Form> page(@RequestParam(defaultValue = "10") int limit, @RequestParam(defaultValue = "1")int offset, String name){
        ...
        return new TableResultResponse<Form>(count,baseBiz.selectByExample(example));
    }
    ...
    public ObjectRestResponse formDesign(@PathVariable  int id, String type, String parseForm){
    	baseBiz.modifyForm(id, type, parseForm);
