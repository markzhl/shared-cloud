var baseForm = {
    baseUrl: "/back/sys/baseForm",
    entity: "baseForm",
    tableId: "baseFormTable",
    toolbarId: "toolbar",
    unique: "id",
    order: "asc",
    currentItem: {}
};
baseForm.columns = function () {
    return [{
        checkbox: true
    }, {
        field: 'id',
...
baseForm.queryParams = function (params) {
    if (!params)
        return {
            name: $("#name").val()
        };
...
baseForm.init = function () {

	baseForm.table = $('#' + baseForm.tableId).bootstrapTable({
        url: baseForm.baseUrl + '/page', //请求后台的URL（*）
        method: 'get', //请求方式（*）
        toolbar: '#' + baseForm.toolbarId, //工具按钮用哪个容器
        striped: true, //是否显示行间隔色
        cache: false, //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
        pagination: true, //是否显示分页（*）
        sortable: false, //是否启用排序
        sortOrder: baseForm.order, //排序方式
        queryParams: baseForm.queryParams,//传递参数（*）
        ...
        uniqueId: baseForm.unique, //每一行的唯一标识，一般为主键列
        showToggle: true, //是否显示详细视图和列表视图的切换按钮
        cardView: false, //是否显示详细视图
        detailView: false, //是否显示父子表
        columns: baseForm.columns()
    });
};
baseForm.select = function (layerTips) {
    var rows = baseForm.table.bootstrapTable('getSelections');
    if (rows.length == 1) {
        baseForm.currentItem = rows[0];
        return true;
    } else {
        layerTips.msg("请选中一行");
        return false;
...
layui.use(['form', 'layedit', 'laydate'], function () {
    ...
    var layerTips = parent.layer === undefined ? layui.layer : parent.layer, //获取父窗口的layer对象
        ...
        baseFormDom = layui.form(),
        layedit = layui.layedit,
        laydate = layui.laydate;
    var addBoxIndex = -1;
    //初始化页面上面的按钮事件
    $('#btn_query').on('click', function () {
        baseForm.table.bootstrapTable('refresh', baseForm.queryParams());
    });
    ...
    $('#btn_add').on('click', function () {
        ...
        $.get(baseForm.entity + '/edit', null, function (baseFormDom) {
            addBoxIndex = layer.open({
                ...
                success: function (layero, index) {
                    var baseFormDom = layui.form();
                    editIndex = layedit.build('formDesc_editor');
                    baseFormDom.render();
                    baseFormDom.on('submit(edit)', function (data) {
                        $.ajax({
                            url: baseForm.baseUrl,
                            type: 'post',
                            data: data.field,
                            dataType: "json",
                            success: function () {
    ...
    $('#btn_edit').on('click', function () {
        if (baseForm.select(layerTips)) {
            var id = baseForm.currentItem.id;
            $.get(baseForm.baseUrl + '/' + id, null, function (data) {
                ...
                $.get(baseForm.entity+'/edit', null, function (baseFormDom) {
                    layer.open({
                        ...
                        success: function (layero, index) {
                            ...
                            baseFormDom.on('submit(edit)', function (data) {
                                $.ajax({
                                    url: baseForm.baseUrl + "/" + result.id,
                                    type: 'put',
                                    data: data.field,
                                    dataType: "json",
                                    success: function () {
    ...
    $('#btn_design').on('click', function () {
        if (baseForm.select(layerTips)) {
            ...
            $.get(baseForm.baseUrl + '/' + id, null, function (data) {
                var result = data.result;
                $.get(baseForm.entity+'/design', null, function (baseFormDom) {
		                        ...
		                        fnCheckForm : function ( type ) {
		                            ...
		                            if(leipiEditor.hasContents()){
		                                ...
		                                console.log(parse_form);
		                                 ...
		                                 $.ajax({
		                                    type: 'POST',
		                                    url : baseForm.baseUrl+"/"+formid+"/design",
		                                    //dataType : 'json',
