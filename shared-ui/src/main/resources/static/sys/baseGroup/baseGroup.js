var baseGroup = {
    baseUrl: "/back/sys/baseGroup",
    entity: "baseGroup",
    tableId: "baseGroupTable",
    toolbarId: "toolbar",
    unique: "id",
    order: "asc",
    currentItem: {},
...
//得到查询的参数
baseGroup.queryParams = function () {
    var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
        name: $("#name").val(),
        groupType: baseGroup.groupType
    };
    return temp;
};
baseGroup.init = function () {
    baseGroup.table = $('#' + baseGroup.tableId).bootstrapTreeTable({
        id: baseGroup.unique,// 选取记录返回的值
        code: baseGroup.code,// 用于设置父子关系
        parentCode: baseGroup.parentCode,// 用于设置父子关系
        rootCodeValue: baseGroup.rootValue,
        url: baseGroup.baseUrl + '/list', //请求后台的URL（*）
        method: 'get', //请求方式（*）
        toolbar: '#' + baseGroup.toolbarId, //工具按钮用哪个容器
        striped: true, //是否显示行间隔色
        cache: false, //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
        ajaxParams: baseGroup.queryParams,//传递参数（*）
        expandColumn: baseGroup.explandColumn,//在哪一列上面显示展开按钮,从0开始
        expandAll: false,
        columns: baseGroup.columns()
    });
};
baseGroup.select = function (layerTips) {
    var rows = baseGroup.table.bootstrapTreeTable('getSelections');
    if (rows.length == 1) {
        baseGroup.currentItem = rows[0];
        return true;
    } else {
        layerTips.msg("请选中一行");
        return false;
...
baseGroup.refresh = function () {
    baseGroup.table.bootstrapTreeTable('refresh', baseGroup.queryParams());
};
...
layui.use(['form', 'layedit', 'laydate', 'element'], function () {
    ...
    $.get(baseGroup.baseUrl + '/all', null, function (data) {
        allItems = data;
    });
    ...
    $.get(baseGroupType.baseUrl + '/all', null, function (data) {
        var first = data[0].id;
        allGroupItems = data;
        for (var i = 0; i < data.length; i++) {
            element.tabAdd('baseGroupType', {
                title: data[i].name
                , content: ""
                , id: data[i].id
            });
        }
        element.tabChange('baseGroupType', first);
    });
    //一些事件监听
    element.on('tab(baseGroupType)', function (data) {
        baseGroup.groupType = $('#baseGroupTypeTab .layui-this').attr("lay-id");
        baseGroup.refresh();
    });
    //初始化页面上面的按钮事件
    ...
    $('#btn_add').on('click', function () {
        ...
        $.get(baseGroup.entity + '/edit', null, function (form) {
            addBoxIndex = layer.open({
                ...
                success: function (layero, index) {
                    ...
                    layero.find("select[name='groupType']").val(baseGroup.groupType);
                    form.render('select');
                    form.on('submit(edit)', function (data) {
                        $.ajax({
                            url: baseGroup.baseUrl,
                            type: 'post',
                            data: data.field,
                            dataType: "json",
                            success: function () {
    ...
    $('#btn_edit').on('click', function () {
        var rows = baseGroup.table.bootstrapTreeTable('getSelections');
        if (baseGroup.select(layerTips)) {
            var id = baseGroup.currentItem.id;
            $.get(baseGroup.baseUrl + '/' + id, null, function (data) {
                ...
                $.get(baseGroup.entity + '/edit', null, function (form) {
                    layer.open({
                        ...
                        success: function (layero, index) {
                            ...
                            form.on('submit(edit)', function (data) {
                                $.ajax({
                                    url: baseGroup.baseUrl + '/' + result.id,
                                    type: 'put',
                                    data: data.field,
                                    dataType: "json",
                                    success: function () {
    ...
    $('#btn_baseUserManager').on("click", function () {
        if (baseGroup.select(layerTips)) {
            var id = baseGroup.currentItem.id;
            $.get(baseGroup.entity + '/user', null, function (form) {
                var index = layer.open({
                    ...
                    success: function (layero, index) {
                        var form = layui.form();
                        // 获取人员
                        $.get(baseGroup.baseUrl + '/' + id + "/user", null, function (data) {
                            if (!data.rel) {
    ...
    $('#btn_baseResourceManager').on("click", function () {
        if (baseGroup.select(layerTips)) {
            var id = baseGroup.currentItem.id;
            var nodeMap = {};
            $.get(baseGroup.entity + '/authority', null, function (form) {
                var index = layer.open({
                    type: 1,
                    title: '分配权限',
                    content: form,
                            ...
                            success: function (defaultData) {
                                ...
                                var $checkableTree = $('#menuTreeview').treeview({
                                    ...
                                    onNodeChecked: function (event, data) {
                                        baseGroup.currentAuthorityMenu = data;
                                        var selectNodes = treeViewHelper.getChildrenNodeIdArr(data);//获取所有子节点
                                        if (selectNodes) {
                                    ...
                                    onNodeUnselected: function(event, data) {
                                        baseGroup.currentAuthorityMenu = {};
                                        authorityResource.refresh();
                                    }
//
                                });
                                ...
                                $.get(baseGroup.baseUrl + '/' + id + "/authority/menu", null, function (data) {
                                    if (data.rel) {
                        ...
                        form.on('submit(edit)', function (data) {
                            ...
                            $.ajax({
                                url: baseGroup.baseUrl + '/' + id + "/authority/menu",
                                type: 'POST',
